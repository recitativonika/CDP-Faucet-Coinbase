(async () => {
    ...
    console.log(`Starting ${totalClaims} claims across ${networks.length} networks, expected to take at least ${totalTimeMinutes.toFixed(1)} minutes.`);
    ...
    for (const network of networks) {
        ...
        for (let i = 0; i < claimsPerNetwork; i++) {
            console.log(`Attempting claim ${i + 1}/${claimsPerNetwork} for ${network}`);
            try {
                await claimFaucet(address, network, 'eth');
            } catch (error) {
                console.error(`Error in claim ${i + 1} for ${network}:`, error);
            }
